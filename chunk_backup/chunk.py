...
class Chunk:
    ...
    def export_grouped_regions(cls, input_region_dir, output_dir, region_groups):
        ...
        for region in region_groups:
            if not os.path.exists(os.path.join(input_region_dir, region)):
                ServerInterface.get_instance().broadcast("区块备份范围内有区块对应的区域文件尚未在世界上生成,请调整备份范围")
                return
        ...
        for region_file, chunks in region_groups.items():
            ...
            for chunk_x, chunk_z in chunks:
                local_x = chunk_x % 32
                local_z = chunk_z % 32
                data = cls.read_chunk_data(input_path, chunk_x, chunk_z)
                if data:
                    chunks_data[(local_x, local_z)] = data

            # 4. 生成新区域文件
            cls.create_region_file(output_path, chunks_data)
            print(f"已生成精简区域文件: {output_path}")

        return True

    @classmethod
    def parse_region_filename(cls, region_filename):
        """
    ...
    @classmethod
    def read_chunk_data(cls, region_file_path, chunk_x, chunk_z):
        """读取区块的原始压缩数据及时间戳（不解压）"""
        local_x = chunk_x % 32
        local_z = chunk_z % 32
    ...
    @classmethod
    def allocate_space(cls, free_sectors, required_sectors, file_handle):
        """优先使用最合适的空闲区域"""
        if not free_sectors:
            # 追加到文件末尾
    ...
    @classmethod
    def create_region_file(cls, output_path, chunks_data):
    ...
    def merge_region_file(cls, source_region_path, target_region_path, overwrite=False, backup_path=None):
        ...
        # 读取源数据
        try:
            source_chunks = cls.read_region_metadata(source_region_path)
        except Exception as e:
            print(f"读取源区域文件失败: {e}")
            return
        ...
        # 解析目标区域坐标（无论是否启用备份）
        try:
            region_x, region_z = cls.parse_region_filename(target_region_path)
        except Exception as e:
        ...
        # 扫描目标文件空闲扇区
        try:
            free_sectors = cls.scan_free_sectors(target_region_path)
        except Exception as e:
        ...
        with open(target_region_path, 'r+b') as target_f:
            for chunk in source_chunks:
                ...
                if existing_sectors > 0 and overwrite and backup_path:
                    ...
                    # 读取备份数据
                    backup_chunk = cls.read_chunk_data(target_region_path, full_chunk_x, full_chunk_z)
                    if backup_chunk:
                ...
                # 分配写入位置
                write_position = cls.allocate_space(free_sectors, chunk['sector_count'], target_f)
        ...
        if backup_path and backup_chunks:
            cls.create_region_file(backup_path, backup_chunks)
    ...
    @classmethod
    def read_region_metadata(cls, region_path):
        chunks = []
        try:
    ...
    @classmethod
    def merge_free_sectors(cls, free_sectors):
        """完全合并相邻或重叠的空闲扇区"""
        if not free_sectors:
            return []
    ...
    @classmethod
    def scan_free_sectors(cls, region_path):
        """空闲扇区扫描"""
        try:
            file_size = os.path.getsize(region_path)
            ...
            return cls.merge_free_sectors(free_sectors)
