...
from chunk_backup.exceptions import ChunkBackupError, FatalError
...
if TYPE_CHECKING:
    from chunk_backup.task import Task
        ...
        if err is not None:
            if isinstance(err, FatalError):
                ...
                if err.on_done:
                    # 先获取主异常的 traceback
                    tb_lines = []
                    # 如果有 causes，将每个 cause 的 traceback 也加上
                    if hasattr(err, 'causes') and err.causes:
                        tb_lines.append("--- Total causes ---\n")
                        for i, cause in enumerate(err.causes):
                            tb_lines.append(f"Cause {i}:\n")
                            tb_lines.extend(traceback.format_exception(type(cause), cause, cause.__traceback__))
                    tb_str = ''.join(tb_lines)
                    broadcast(tr("other.error.unknown", name=self.task.tr("name"), error=tb_str))
...
class TaskQueue(Generic[_T]):
    class TooManyOngoingTask(ChunkBackupError):
